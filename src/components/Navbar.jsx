...
const Navbar = () => {
  return (
    <nav className="flex w-full h-[112px] md:h-28 justify-between items-center px-8 py-4 bg-white/25 backdrop-blur-sm border border-white/18 fixed z-20">
          ...
          {["home", "about", "work", "blog", "testimonials", "contact"].map(
            (item) => (
              <li key={`link-${item}`} className="my-0 mx-4 flex-col group">
                ...
                <Link
                  href={`#${item}`}
                  className=" text-lg uppercase flex-col font-semibold p-1 transition-all ease-in-out text-secondary"
                >
                  {item}
                </Link>
              </li>
