  ...
  return (
    <section className="container bg-slate-100 mx-auto grid grid-cols-1 min-h-fit">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-5 gap-y-10 h-full  ">
        {posts.map((post) => (
          <Link href={post.slug.current} key={post._id} className="group">
            <div className="relative overflow-hidden bg-white rounded-lg duration-300 ease-in-out m-4 shadow-md hover:shadow-lg hover:translate-y-[-3px] transition-transform flex flex-col min-h-full w-4/5 xs:w-2/3 sm:w-3/4 md:w-3/4 xl:w-4/5 2xl:w-5/6 mx-auto">
              ...
              <div className="flex-1 flex flex-col justify-between">
                ...
                <div className="bg-lightGray text-center text-gray group-hover:bg-secondary group-hover:text-white">
                  Read more
                </div>
              </div>
            </div>{" "}
      ...
      <div className="flex justify-center mt-5 mb-7">
        <a
          href="#"
          className="font-semibold text-xl px-8 py-2 text-black rounded-full mt-8 hover:bg-secondary hover:text-white"
        >
          All Posts
        </a>
      </div>
